...
2. DietPi désactive le support caméra par défaut pour économiser les ressources. Avant de pouvoir utiliser la caméra, j’ai activé le support avec la commande `dietpi-set_hardware rpi-camera enable`, qui augmente la mémoire GPU à 96 Mo et retire les modules caméra de la liste noire. J’ai aussi ajouté `camera_auto_detect=1` et `dtoverlay=vc4-kms-v3d` dans `/boot/firmware/config.txt`, ce dernier étant essentiel pour que le stack libcamera moderne fonctionne.
...
- **Forces** : La caméra s’intègre nativement à l’écosystème Raspberry Pi via libcamera, sans driver tiers à compiler. L’autofocus PDAF est un vrai avantage : en éclairage normal, les images sont <mark>nettes sans ajustement manuel</mark>. Picamera2 offre une API Python claire et bien documentée.
